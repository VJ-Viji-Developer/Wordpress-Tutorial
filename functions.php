...
//    same we can do for image section10 16:55
...
function university_features(){
    ...
    // register_nav_menu('footermenuloaction1','Footer Menu Location1');
    // register_nav_menu('footermenuloaction2','Footer Menu Location2');
}
...
function university_adjust_queries($query){
    ...
    if(!is_admin() AND is_post_type_archive('event') AND $query->is_main_query ){
        $today=date('Ymd');
        $query->set('mata_key','event_date');
        $query->set('orderby','meta_value_num');
        $query->set('order','ASC'); 
        $query->set('mata_query', array(
            array(
            'key'=>'event_date',
            'compare'=>'>=',
            'value'=>$today,
